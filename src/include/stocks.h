...
class trade
{
  public:
    ...
    int quantity_of_shares() const noexcept;

    /** @brief Were shares bought or sold */
    buy_sell_indicator buy_sell() const noexcept;

    /** @brief The total price of the trade */
    double traded_price() const noexcept;
...
double volume_weighted_stock_price(Iterator begin, Iterator end) noexcept
{
 ...
 */
template <typename Iterator> double gbce_all_share_index(Iterator begin, Iterator end) noexcept
{
#ifndef NDEBUG
 ...
 */
template <typename Iterator>
double gbce_all_share_index_stable(Iterator begin, Iterator end) noexcept
{
#ifndef NDEBUG
    for (auto it = begin; it != end; ++it)
    {
